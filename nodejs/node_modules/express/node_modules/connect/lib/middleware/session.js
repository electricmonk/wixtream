...
function session(options){
  ...
  store.generate = function(req){
    ...
    req.pending.cookie = new Cookie(cookie);
  };

  return function session(req, res, next) {
    // self-awareness
    if (req.pending) return next();
    ...
    res.on('header', function(){
      ...
      // compare hashes
      } else if (originalHash == hash(req.pending)) {
    ...
    res.end = function(data, encoding){
      res.end = end;
      if (!req.pending) return res.end(data, encoding);
      debug('saving');
      req.pending.resetMaxAge();
      req.pending.save(function(){
        debug('saved');
